# 袁超
# 开发时间：2026/5/22 14:22
# 袁超
# 开发时间：2026/5/22 13:27
# -*- coding: utf-8 -*-
"""
...
def train_and_predict_rnn(rnn, get_params, init_rnn_state, num_hiddens, vocab_size, device, corpus_indices, idx_to_char,
                          char_to_idx, is_random_iter, num_epochs, num_steps, lr, clipping_theta,
...
def train_ch8(train_iter, test_iter, net, loss, optimizer, device, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        test_acc = evaluate_accuracy(test_iter, net)
        print('epoch %d, loss %.4f, train acc %.3f, test acc %.3f, time %.1f sec' % (
        epoch + 1, train_l_sum / batch_count, train_acc_sum / (n * 153600.0), test_acc / 153600.0, time.time() - start))
...
# 自定义单GPU/CPU通用训练函数（替换d2l有问题的train）
def custom_train(train_iter, test_iter, net, loss, optimizer, device, num_epochs):
    print("training on:", device)
    for epoch in range(num_epochs):
