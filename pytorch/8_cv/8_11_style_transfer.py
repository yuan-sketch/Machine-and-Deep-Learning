...
# VGG 网络使用了5个卷积层
# 选择第四个卷积层的最后一个卷积层作为内容层
# 每个卷积的第一个卷积层作为样式层
# 构建net,只保留需要用到的VGG的所有层
net_list = []
...
# 用gram矩阵表示样式层输出的样式
def gram(X):
    num_channels, n = X.shape[1], X.shape[2]*X.shape[3]       #通道数，高，宽
    X = X.view(num_channels, n)
    return torch.matmul(X, X.t()) / (num_channels*n)
def style_loss(Y_hat, gram_Y):
    return F.mse_loss(gram(Y_hat),gram_Y)
# 降低总变差损失，尽可能使邻近的像素值相似
def tv_loss(Y_hat):
...
# 将内容图像和样式图像的高和宽调整为150和225，合成图像由内容图像标准化
image_shape = (150,225)
net = net.to(device)
